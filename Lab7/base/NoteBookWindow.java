	...
	private HBox addHBox() {
        ...
        Search.setOnAction(new EventHandler<ActionEvent>(){
            ...
            public void handle(ActionEvent event){
                currentSearch = EnterSearch.getText();
                textAreaNote.setText("");
                List<Note> res = new ArrayList<Note>();
                res = noteBook.searchNotes(currentSearch);

                ObservableList<String> stringRes = FXCollections.observableArrayList();
                for (Note n: res){
                    stringRes.add(n.getTitle());
                }
                titleslistView.setItems(stringRes);

            }
        });

        Button ClearSearch = new Button("ClearSearch");
		ClearSearch.setPrefSize(100, 20);
		ClearSearch.setOnAction(new EventHandler<ActionEvent>(){
            ...
            public void handle(ActionEvent event){
                ...
                //set titles to current folder
                updateListView();
            }
        });

		hbox.getChildren().addAll(buttonLoad, buttonSave, SearchTxt, EnterSearch, Search, ClearSearch);
